...
//        //获取用户拥有的角色 权限检查 todo
        ...
        User user = new User(userInfo.getUserId(), userInfo.getPassword(),
                grantedAuths);
        ...
        //使用JWT 代码
        UserDetail userDetail = DozerBeanMapperUtil.getObject(userInfo, UserDetail.class);
        userDetail.setUserId(userInfo.getUserId());
        userDetail.setStatus(1);


        return JWTUserDetailsFactory.create(user, userDetail);
    }
